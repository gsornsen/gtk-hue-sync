    ...
    def get_dominant_color(self):
        ...
        with Monitor() as monitor:
            ...
            if self.fullscreen is True:
                screen_grab = ImageGrab.grab(bbox=(monitor.x, monitor.y,
                                                   monitor.width,
                                                   monitor.height),
                                             childprocess=False)
            else:
                ...
                y_end = (y_start + 100)
                screen_grab = ImageGrab.grab(bbox=(monitor.x, int(y_start),
                                                   monitor.width, int(y_end)),
                                             childprocess=False)
            color_thief = ColorThief(screen_grab, True)
            try:
                dominant_color = color_thief.get_color(self.ignore_white,
                                                       self.ignore_black,
                                                       self.quality)
